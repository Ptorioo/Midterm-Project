...
public:
    ...
    void redecideShift(int num, int day, Schedule& schedule){
        ...
        employeeBestContribution[num][day] = make_pair(0, 0);
        update(num, day, schedule);
        int bestCon = employeeBestContribution[num][day].first, bestShift = -employeeBestContribution[num][day].second;
        if(bestCon > 0) schedule.assignShift(num, day, bestShift, bestCon);
    }
};
