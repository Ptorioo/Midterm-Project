...
public:
    const int &nI, &nJ, &nK, &L, &w1, &w2;
    vector<vector<bool>>& request;
    vector<vector<tuple<int,int,bool,int>>> employeeBestContribution;

    Employee_Random(int& _nI, int& _nJ, int& _nK, int& _L, int& _w1, int& _w2, vector<vector<bool>>& _request): nI(_nI), nJ(_nJ), nK(_nK), L(_L), w1(_w1), w2(_w2), request(_request){
        employeeBestContribution.resize(nI);
    ...
    void update(int num, int day, Schedule& schedule){
        ...
        for(int j = 1; j <= schedule.nK; j++){
            ...
            employeeBestContribution[num][day] = max(employeeBestContribution[num][day], make_tuple(tmpCon, rand(), !isNightShift(j, schedule.shiftType), -j));
        }
    }
...
int main(){
    ...
    Employee_Random employee2(nI, nJ, nK, L, w1, w2, request);
